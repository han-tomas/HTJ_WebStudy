...
/*
	...
	model 2 (JSP / Java) => 단점 : 복잡성 / 장점 : 여러명이 동시에 개발, 
								 			  추가, 확장성, 재사용 용이
						 => Controller에 집중이 된다		 			  
    ...
    * Controller를 찾는 방법 : .do (URL주소 설정)
    			*.do 
    			...
    			board/insert.do
    			board/category.do
   	사용자 요청 ------------------- Controller (DiapatcherServlet) ----------- Model ------ DAO
   								 (Front Controller)
   								 		|
   	...
   	JSP => JSP (Model) => 홈페이지, 소기업 : 유지보수가 없는 경우
   	JSP => 자바 => JSP => MV
...
public class DispatcherServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 1. 사용자가 보내준 URI 주소를 읽어 온다
		String cmd = request.getRequestURI();
		//http://localhost:8080/JSPAjaxProject3/diary3/diary.do
		//                      ------------------------------- URI
