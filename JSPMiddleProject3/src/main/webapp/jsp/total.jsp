...
<%--
	...
	표현식)
		...
		내장 객체 (일반 JSP ==> 자바/HTML 분리)
				 ...
				 자바/HTML혼합   
			= requestScope 
				기존의 request + 추가 데이터 설정 => request.setAttribute("key", 값) : 오라클에서 얻은 값을 추가
			...
			= paramValues :&{parValues.key} => request.getParameterValues("key")
			${자바의 일반 변수 사용이 안된다}
			String name="";
			${name} => 출력이 안됨
