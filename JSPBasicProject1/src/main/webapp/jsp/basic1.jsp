<%--
	...
	------------------------------------
		***웹 프로그램 : C/S => cloud (CI/CD)
		   client / server
		   	 |		  |
		   	 |		Java/Oracle/Spring/Spring_boot
		   	 |			 ------ JDBC=>MyBatis=>JPA
		  CSS/HTML/JavaScript 
	1. 구성요소
		Client : 브라우저
	...
	3. Servlet VS JSP
		...
		=> Servlet은 자바파일 => 수정시마다 컴파일을 해야 한다.
			1. 변경시마다 컴파일
			2. HTML이 자바안에서 코딩 => 문자열 out.print()
			3. CSS를 구사하기가 어렵다 => 개발자(HTML,JS,CSS,Java,Oracle...)
		=> 위 단점을 보안 => JSP(HTML을 쉽게 만든다.)
		=> JSP장점
			서블릿보다 쉽고 작성하기 빠르다
			HTML과 Java가 분리되어 있다.
			------------ 분업해서 사용이 가능
			=> 컴파일하지 않고 바로 확인기 가능하다
	4. JSP 주로 사용하는 위치
	...
	4. JSP
		...
		_jspInit() : web.xml => 저장된 내용 읽어서 저장
					 error 코드, 서블릿 저장, 환경 설정 위치, 한글 변환
		|
