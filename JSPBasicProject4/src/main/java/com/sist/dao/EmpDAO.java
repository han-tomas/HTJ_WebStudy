...
public class EmpDAO {
	...
	public void empInsert(EmpVO vo)
	{
		try
		{
			...
			// -----------                          ---------
			// ''를 자동으로 붙여준다 => table명, 컬럼명에는 사용하지 않는다.
			// MyBatis => $, #
	...
	public List<Integer> empGetMgrData()
	{
		...
		{
			conn=db.getConnection();
			String sql="SELECT DISTINCT mgr FROM myEmp"; // DISTICT 중복을 제거
			ps=conn.prepareStatement(sql);
	...
	public List<Integer> empGetSalData()
	{
		...
		{
			conn=db.getConnection();
			String sql="SELECT DISTINCT sal FROM myEmp"; // DISTICT 중복을 제거
			ps=conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while(rs.next())
			{
	...
	public List<Integer> empGetDeptnoData()
	{
		...
		{
			conn=db.getConnection();
			String sql="SELECT DISTINCT deptno FROM myEmp"; // DISTICT 중복을 제거
			ps=conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while(rs.next())
			{
	...
	public List<String> empGetJobData()
	{
		...
		{
			conn=db.getConnection();
			String sql="SELECT DISTINCT job FROM myEmp"; // DISTICT 중복을 제거
			ps=conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while(rs.next())
			{
	...
	public List<EmpVO> empListData()
	{
		...
		{
			conn=db.getConnection();
			// 첫번째 문제 => ANSI->오라클 조인으로 바꿔서 제출
			String spl="SELECT empno,ename,job,TO_CHAR(hiredate,'YYYY-MM-DD'),TO_CHAR(sal,'L999,999'),NVL(comm,0),dname,loc,grade "
	...
	public EmpVO empDetailData(int empno)
	{
		...
		try
		{
			...
			String sql2="SELECT empno, ename, job, NVL(mgr,0), TO_CHAR(hiredate,'YYYY-MM-DD'),TO_CHAR(sal,'L999,999'),NVL(comm,0),"
					+ "(SELECT dname FROM myDept WHERE deptno=me.deptno) AS dname,"
					+ "(SELECT loc FROM myDept WHERE deptno=me.deptno) AS loc,"
	...
	// 수정 데이터 읽기
	public EmpVO empUdateData(int empno)
	{
		EmpVO vo= new EmpVO();
		try
		{
