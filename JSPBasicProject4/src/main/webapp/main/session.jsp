...
<%--
	request/ response/ session => 웹 개발의 핵심
					   ------- 뜻 : 프로그램 실행 하는 중
	session은 서버에 저장 (실행하는 동안에만 유지) : 사용자의 정보를 지속적으로 관리
	------- 1) 장바구니, 결재, 예약, 추천 ...
	...
	주요메소드
			...
			getAttribute(): session에 저장된 데이터 읽기
			removeAttribute(): session에 저장된 데이터 일부를 지울때 사용	
 --%>    
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<table class="table">
		<tr>
			<td width=20% class="text-cneter">클래스명</td>
		...
		<tr>
			...
			<td width=80%>
				<ul>
					...
					<li>클ㄹ라이언트당 1개의 session공간 : getId():websocket</li>
