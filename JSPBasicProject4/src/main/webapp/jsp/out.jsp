		...
		request : HttpServletRequest
					...
					= 요청정보
						* getParameter() : 사용자 전송한 데이터를 한개만 받는다
							=> 문자열 : String 
						* getParametetValues() : 여러개를 동시에 받을 경우
							=> String[] : 체크박스 select의 멀티 ...
						* setCharacterEncoding() : 디코딩
							=> POST에서만 사용
							=> GET은 자동화 처리 => window10 이상
							   ============= server.xml(한글처리, 포트)
							   8080은 프록시서버
		...
		response : 응답 정보, 화면 이동 정보 => HttpServletResponse
				   ...
				   ----------
				   	필요시에 서버에서 화면을 요청 화면이 아닌 다른 화면 이동
				   		  ...
				   		  delete => list
				   		  update => detail
				   	=> sendRedirect() => GET방식, request를 초기화한 후 이동
...
<%
        ...
        for(EmpVO vo:list)
        {
            ...
            out.print("<td class=text-center><a href=\"EmpDetailServlet?empno="+vo.getEmpno()+"\">"+vo.getEname()+"</a></td>");
