 ...
 *           id		name		sex		age
 *       ------------------------------------------
 *          aaa 	홍길동		남자		20		| first() :커서위치를 처음으로 => next()
 *          ↙		  ↓			 ↓         ↘	  위치변경 					위치변경 후 데이터 읽기
 ...
 *  getString(1) getString(2) getString(3) getInt(4)        뒤에서부터 데이터 읽기
 *       ------------------------------------------			
 *       | 커서위치
 *       
 *       
...
public class FoodDAO{
	 ...
	 *    			"'"+name+"','"+sex+"','"+ ...
	 *    PreparedStatement => 미리 SQL문장을 만들고 나중에 값을 채운다 => default
	 *    
	...
	public void foodCategoryInsert(CategoryVO vo)
	{
		try
		{
			...
			/* 
			 * 단점 => 번호가 잘못되면 오류 발생 ==> ? 갯수와 번호가 동일해야한다
			 *        데이터형이 다르면 오류 발생
			 */
		...
		{
			disConnection(); // 오라클 연결 해제 => 무조건 시행
		}
	}
	// 1-1. 실제 맛집 정보 저장
	/*
