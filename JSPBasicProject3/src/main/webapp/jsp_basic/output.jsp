    	...
    	<jsp:setProperty name="bean" property="*"/>
  		...
  		---------------------------------------------------------
  		    ...
  		    속성
  		    	...
  		    	class : 클래스명
  		    	scope : 사용범위
  		    		= page (defaul) => 한개 JSP에서 사용(다른 페이지 이동 => 메모리 해제)
  		    		  *** 생략이 가능
  		    ...
  		    <jsp:useBean id="a" class="A">
  		      ...
  		      class id	
  		     	==> a.setName("홍길동")
  		      	...
  		      	a.setNo(Integer.parsInt(request.getParameter("no"))
  		     </jsp:useBean>
  		---------------------------------------------------------
  		    <jsp:getProperty> : 읽기										  	
  		    => getter
  		    <jsp:setProperty name="a" property="name"> => a.getName()
  		    <jsp:setProperty name="a" property="no"> => a.getNo()
  		---------------------------------------------------------
  			useBean : 메모리 할당
  			setProperty : setXxx()
  			getProperty : getXxx()
