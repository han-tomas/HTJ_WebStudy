...
<%-- 
	...
	----
	 방법 => 1. Connection 객체 생성(maxActive: 최대,maxIdle:풀)
	 	    ...
	 	    2. 코딩 방법
	 	       ------
	 	       1) getConnection() : 미리 생성된 Connection 객체 열기
	 	       2) disConnection() : 반환
	 	       3) 기능 => 이전하고 동일
	 	       *** 보안이 좋다 
 ...
 <%
 	...
 	// 2. 실행과 동시에 페이지 전송이 불가능 => 첫 페이지는 default 설정
 	if(strPage==null)
 		strPage="1";
 	// 3. 현재 페이지 지정
...
<body>
            ...
            <tr>
               <td>
                  <form method="post" action="food_find.jsp" id="frm">
                     <input type="text" id="addr" name="addr" size=20 class="input-sm"
                        value="<%=fd%>"> <input type="button" id="findBtn" value="검색"
      ...
      <div class="row">
         ...
         <div class="col-md-3">
            <div class="thumbnail">
               <a href="#" target="_blank"> <img src="<%=vo.getPoster()%>"
                  style="width: 100%">
                  <div class="caption">
                     <p><%=vo.getName()%></p>
                     <p><%=vo.getAddress()%></p>
